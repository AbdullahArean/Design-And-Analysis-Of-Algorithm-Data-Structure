/*
Problem Link   : https://youtu.be/3RhGdmoF_ac
About Problem  : Aditya Verma Video Lecture 15
Tags           : Binary Search
Notes          :
...
int mindiffelemnt(int arr[], int N, int key)
{
    if(BinarySearch(arr,N,key)!=-1){
        ...
        if(index==0) return abs(arr[0]-key);
        else if(index==N-1) return abs(arr[index]-key);
        else if(abs(arr[index-1]-key)<=abs(arr[index]-key)){

            return abs(arr[index-1]-key);
            //return arr[index-1]; //if the element needed
        }
        else abs(arr[index]-key);
        //else return arr[index]; //if the element needed
    }
    return -1;
}

void Solution(){
    int arr[] = {1,20,30,45,50,60,70};
    int N= sizeof(arr)/sizeof(arr[0]);
    
    cout<<mindiffelemnt(arr,N,-2)<<endl;
